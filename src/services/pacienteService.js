...
const consultasPorPaciente = async (id) => {
  const [linhas] = await db.query(
    'SELECT c.id AS consulta_id, c.data_hora, c.status, p.nome AS nome_paciente, p.email AS email_paciente, m.nome AS nome_medico, m.especialidade, m.email AS email_medico FROM Consultas AS c JOIN Pacientes AS p ON c.paciente_id = p.id JOIN Medicos AS m ON c.medico_id = m.id WHERE c.paciente_id = ?',
    [id]
  );
  return linhas;
};
